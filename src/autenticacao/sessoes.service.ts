...
export class SessoesService implements OnModuleInit, OnModuleDestroy {
  ...
  async consumir(token: string): Promise<number> {
    ...
    if (typeof consumida !== 'object' || consumida === null || !('corretor_id' in consumida) || !Number.isInteger(Number(consumida.corretor_id)) || Number(consumida.corretor_id) < 1
    ...
    return Number(consumida.corretor_id);
  }
