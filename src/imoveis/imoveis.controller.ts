...
export class ImoveisController {
  ...
  @Get(':id') encontrar(@Param('id', ParseUUIDPipe) id: string) { return this.imoveis.encontrar_interno(id); }
  @Post() criar(@Body() dto: CriarImovelDto, @Req() requisicao: Requisicao) { return this.imoveis.criar(dto, requisicao.user); }
  @Patch(':id') atualizar(@Param('id', ParseUUIDPipe) id: string, @Body() dto: AtualizarImovelDto, @Req() requisicao: Requisicao) { return this.imoveis.atualizar(id, dto, requisicao.user); }
  @Delete(':id') @HttpCode(204) desativar(@Param('id', ParseUUIDPipe) id: string, @Req() requisicao: Requisicao) { return this.imoveis.desativar(id, requisicao.user); }
}
